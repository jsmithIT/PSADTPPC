...
Try {
	...
	[string]$appVendor = 'Jackie Smith'
	[string]$appName = 'Package Priority Calculator'
	[string]$appVersion = '1.0.0'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '2022.09.14'
	...
	$CloseApps = "cmd=Command Prompt"

	##* Application install parameters.
	# 64-bit application
	# Application install Path.
	$64bitAppInstallPath = "$PSScriptRoot\Files\Package Priority Calculator"

	##* Application Settings Directory
	# Directory where application settings be reside.
	$appSettingsDir = ("$Env:ProgramData\Tuolumne County\Application Settings\Package Priority Calculator")
	...
	$64bitAppInstallLocation = "$Env:Programfiles\Package Priority Calculator"

	# Start menu shortcut path
	$MenuShortcutPath = "$Env:ProgramData\Microsoft\Windows\Start Menu\Programs\Package Priority Calculator.lnk"

	# Desktop shortcut path
	$DesktopShortcutPath = "${Env:PUBLIC}\Desktop\Package Priority Calculator.lnk"
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Try {
			...
			New-Shortcut -Path "$MenuShortcutPath" -TargetPath "$64bitAppInstallLocation\$apName.bat" -Description "$apName" -IconLocation "$64bitAppInstallLocation\Assets\Logo.ico"
		...
		New-Shortcut -Path "$DesktopShortcutPath" -TargetPath "$64bitAppInstallLocation\$apName.bat" -IconLocation "$64bitAppInstallLocation\Assets\Logo.ico" -WindowStyle "Maximized" -Description "$apName"
